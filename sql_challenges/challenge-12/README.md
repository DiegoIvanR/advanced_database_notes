...
-- EXERCISE 1: Define "Team Velocity"
-- ============================================================
-- 1. Business question: Which teams are completing work fastest and how do they compare?
-- 2. Exact definition: Count of 'completed' tasks per team, normalized by team member count 
--    (tasks/person) to keep the comparison fair across different team sizes.
-- 3. Edge cases: Teams with zero members or zero completed tasks must return 0 instead of dropping out.
-- 4. Unit: Tasks completed per team member.
-- 5. Misleading trigger: Ignores task complexity. A team fixing 10 tiny typos looks "faster" 
--    than a team shipping 1 massive database migration.
-- ============================================================
...
-- EXERCISE 2: Define "On-Time Delivery Rate"
-- ============================================================
-- 1. Business question: Are we meeting our promised task deadlines?
-- 2. Exact definition: Percentage of 'completed' tasks where completed_at <= due_date + 1 (end of day).
-- 3. Edge cases: Excludes tasks with no due_date, open tasks, and cancelled tasks.
-- 4. Unit: Percentage (%).
-- 5. Misleading trigger: If teams intentionally set artificially late due_dates, 
--    the delivery rate climbs to 100%, masking poor speed.
-- ============================================================
...
-- ============================================================
-- PROBLEM: 
-- 1. It counts ALL tasks linked to a user, meaning someone who opens 100 
--    tasks but completes 0 looks like an absolute superstar.
-- 2. It treats a critical infrastructure overhaul exactly the same as a spelling fix.
-- 3. It uses an INNER JOIN, meaning employees with no current tasks are completely hidden.
--
-- REWRITE: Completed tasks per user, weighted by priority points.
-- ============================================================
...
-- ============================================================
-- PROBLEM: 
-- It takes the arithmetic `AVG()` of Primary Key IDs (`ts.id`). 
-- A task ID is an arbitrary surrogate sequence number (e.g., ID 40023). 
-- Averaging IDs yields a completely meaningless mathematical scalar.
--
...
-- ============================================================
-- PROBLEM: 
-- 1. `priority` is a string data type ('high', 'medium'). Multiplying a 
--    VARCHAR2 text block by 10 throws an invalid number compilation error.
-- 2. It attempts to add a date object directly to that mathematical scalar, 
--    which violates strong typing protocols.
--
-- REWRITE: Assign a numeric map to priority and calculate days remaining until due.
-- ============================================================
...
ORDER BY true_urgency_score DESC;